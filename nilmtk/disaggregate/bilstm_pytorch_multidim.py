...
class BiLSTM_Pytorch_MUltichannel(nn.Module):
    def __init__(self, sequence_length, ndim = 2):
        ...
        '''
        super(BiLSTM_Pytorch_MUltichannel, self).__init__()
        self.seq_length = sequence_length
        self.pad = nn.ConstantPad1d((1, 2), 0)
        self.conv = nn.Conv1d(ndim, 16, 4, stride = 1)
...
def train(appliance_name, model, mains, appliance, epochs, batch_size, pretrain,checkpoint_interval = None,  train_patience = 3):
    ...
    for epoch in range(epochs):
        ...
        if best_loss is None or final_loss < best_loss:
            ...
            path_state_dict = "./"+appliance_name+"_mul_bilstm_best_state_dict.pt"
        ...
        if (checkpoint_interval != None) and ((epoch + 1) % checkpoint_interval == 0):
            checkpoint = {"model_state_dict": model.state_dict(),
                            ...
                            "epoch": epoch}
            path_checkpoint = "./"+appliance_name+"_mul_bilstm_checkpoint_{}_epoch.pkl".format(epoch)
...
class MUL_BiLSTM(Disaggregator):

    def __init__(self, params):
        self.MODEL_NAME = "MUL_BiLSTM"
    ...
    def partial_fit(self,train_main,train_appliances,pretrain = False, do_preprocessing=True,  **load_kwargs):
        ...
        for appliance_name, power in train_appliances:
            if appliance_name not in self.models:
                ...
                self.models[appliance_name] = BiLSTM_Pytorch_MUltichannel(self.sequence_length, ndim = train_main.shape[2])
                # Load pretrain dict or not
                if pretrain is True:
                    self.models[appliance_name].load_state_dict(torch.load("./"+appliance_name+"_mul_bilstm_pre_state_dict.pt"))

            model = self.models[appliance_name]
            train(appliance_name,model, train_main, power, self.n_epochs, self.batch_size, pretrain, checkpoint_interval = None)
            # Model test will be based on the best model
            self.models[appliance_name].load_state_dict(torch.load("./"+appliance_name+"_mul_bilstm_best_state_dict.pt"))
